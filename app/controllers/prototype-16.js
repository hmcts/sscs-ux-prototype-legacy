...
var controller = {
    ...
    appointee: function(req, res) {
        if (req.body.radioGroup === 'appointee') {
            res.render('prototype-beta-16/submit-your-appeal/004-appointee-details');
        } else if (req.body.radioGroup === 'nonAppointee') {
            res.render('prototype-beta-16/submit-your-appeal/004-your-details');
        }
    },

    representative: function(req, res) {
        if (req.body.radioGroup === 'rep') {
            res.render('prototype-beta-16/submit-your-appeal/009-reps-details');
        } else if (req.body.radioGroup === 'noRep') {
            res.render('prototype-beta-16/submit-your-appeal/011-why-are-you-appealing');
        }
    },
    ...
    supporter: function(req, res) {
        if (req.body.radioGroup === 'yes') {
            res.render('prototype-beta-16/submit-your-appeal/010-supporter-details');
    ...
    hearing: function(req, res) {
        if (req.body.radioGroup === 'attend') {
            res.render('prototype-beta-16/submit-your-appeal/014-hearing-arrangements-needed');
        } else if (req.body.radioGroup === 'notAttend') {
            res.render('prototype-beta-16/submit-your-appeal/013-hearing-not-attending');
        }
    },

    arrangements: function(req, res) {
        if (req.body.radioGroup === 'arrangementsRequired') {
            res.render('prototype-beta-16/submit-your-appeal/015-hearing-arrangements');
        } else if (req.body.radioGroup === 'arrangementsNotRequired') {
            res.render('prototype-beta-16/submit-your-appeal/016-hearing-dates');
        }
    },
    ...
    mobile: function(req, res) {
        if (req.body.radioGroup === 'usemobile') {
            res.render('prototype-beta-16/submit-your-appeal/007-sms-confirmation');
        } else if (req.body.radioGroup === 'newmobile') {
            res.render('prototype-beta-16/submit-your-appeal/006-enter-mobile-number');
        }
    },

    mobileboth: function(req, res) {
        if (req.body.radioGroup === 'usemobileboth') {
            res.render('prototype-beta-16/submit-your-appeal/007-sms-email-confirmation');
        } else if (req.body.radioGroup === 'newmobileboth') {
            res.render('prototype-beta-16/submit-your-appeal/006-enter-mobile-number-both');
        }
    },

    notifications: function(req, res) {
        if (req.body.radioGroup === 'email') {
            res.render('prototype-beta-16/submit-your-appeal/006-enter-email');
        } else if (req.body.radioGroup === 'text') {
            res.render('prototype-beta-16/submit-your-appeal/006-mobile-number-provided');

        } else if (req.body.radioGroup === 'both') {
            res.render('prototype-beta-16/submit-your-appeal/006-enter-email-both');

        } else if (req.body.radioGroup === 'neither') {
            res.render('prototype-beta-16/submit-your-appeal/008-representative');
        }
    },
...
// To create a moment object from user input (e.g. MRN date) you would do the following:
        ...
        if (DateUtils.isLessThanOrEqualToAMonth(mDate)) {
            res.render('prototype-beta-16/submit-your-appeal/004-appointee');
            return;
        }

        if (DateUtils.isGreaterThanAMonth(mDate) && DateUtils.isLessThanOrEqualToThirteenMonths(mDate)) {
            res.render('prototype-beta-16/submit-your-appeal/003-3-why-appeal-late-1month');
            return;
        }

        if (DateUtils.isGreaterThanThirteenMonths(mDate)) {
            res.render('prototype-beta-16/submit-your-appeal/003-4-why-appeal-late-13months');
            return;
        }
    },
...
validateCaseReference: function (req, res) {
    ...
    if(surname === 'wrong') {
        ...
        res.render('prototype-15/track-your-appeal-error-no-match', {
    ...
    if(errors.reference.length || errors.surname.length) {
        res.render('prototype-15/track-your-appeal-error', {
            errorMessages: errors,
            caseRefNumber: reference,
            surname: surname
        });
    ...
    .then(function (result, error) {
        ...
        res.render('prototype-15/status-pb-one', data);
                //res.render('prototype-15/status-pb-four', data);
                //res.render('prototype-15/status-pb-seven', data);
                //res.render('prototype-15/status-pb-ten', data);
            }, function (error) {
                res.send('HTTP '  + error.status + ': ' + error.message);
            });
...
validateCaseReferenceMulti: function(req, res) {
    ...
    if(caseRef1 === '' && caseRef2 === '' && caseRef3 === '') {
        ...
        res.render('prototype-15/track-your-appeal-error-multi', {
            errorMessages: errors,
    ...
    .then(function (result, error) {
        var data = result.body;
        data.status = data.status.toLowerCase().split('_').join('');
        res.render('prototype-15/status', data);
    }, function (error) {
        res.send('HTTP '  + error.status + ': ' + error.message);
    });
},
...
validateReference: function(req, res) {
    ...
    if(errors.length) {
        res.render('prototype-15/validate-reference-error', {
            errors: errors,
            reference: reference
        });
        return;
    ...
    res.render('prototype-15/validate-surname');
},
...
validateSurname: function(req, res) {
    ...
    if(errors.length) {
        res.render('prototype-15/validate-surname-error', {
            errors: errors,
            reference: reference,
            surname: surname
        });
